...
public final class CryptoHelper implements ICryptoHelper {
	...
	public byte[] scrypt(byte[] input, byte[] salt, int cost) {
		return SCrypt.generate(input, salt, clamp(1, 2^128, upperPowerOfTwo(cost)), 8, 1, 128);
	}
	public synchronized byte[] phpass(byte[] input) {
		return phpass(input, UTF_8);
	}
