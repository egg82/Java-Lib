...
public class ExpiringRegistry<K, V> implements IExpiringRegistry<K, V> {
	...
	private final EventHandler<RegisterExpireEventArgs<K, V>> expire = new EventHandler<RegisterExpireEventArgs<K, V>>();
	...
	private final ExpirationListener<K, Unit<V>> onExpiration = new ExpirationListener<K, Unit<V>>() {
		public void expired(K key, Unit<V> value) {
			...
			keysDirty.set(true);
			expire.invoke(this, new RegisterExpireEventArgs<K, V>(key, value.getType()));
		}
	};
}
