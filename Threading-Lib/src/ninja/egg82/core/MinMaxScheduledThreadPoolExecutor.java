...
public class MinMaxScheduledThreadPoolExecutor implements ScheduledExecutorService {
    ...
    public <T> Future<T> submit(Callable<T> task) {
        if (core != null) {
            ...
            if (extra != null) {
                // We have extra
                if (core.getTaskCount() - core.getCompletedTaskCount() >= core.getCorePoolSize()) {
                    // Core is maxed out
                    if (extra.getTaskCount() - extra.getCompletedTaskCount() >= extra.getCorePoolSize() && extra.getCorePoolSize() < extraMax) {
    ...
    public <T> List<Future<T>> invokeAll(Collection<? extends Callable<T>> tasks) throws InterruptedException {
        if (core != null) {
            ...
            if (extra != null) {
                ...
                int coreActiveCount = (int) (core.getTaskCount() - core.getCompletedTaskCount());
                ...
                if (coreActiveCount >= core.getCorePoolSize()) {
                    ...
                    if (extra.getCorePoolSize() < extraMax && extra.getTaskCount() - extra.getCompletedTaskCount() >= extra.getCorePoolSize()) {
                        // Extra is entirely active, but not maxed out
    ...
    private <T> Callable<T> getHandlerWrapper(Callable<T> task) {
        return new Callable<T>() {
            public T call() throws Exception {
                ...
                if (extra.getTaskCount() - extra.getCompletedTaskCount() >= extra.getCorePoolSize()) {
